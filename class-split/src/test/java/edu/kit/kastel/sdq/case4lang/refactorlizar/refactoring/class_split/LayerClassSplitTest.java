...
public class LayerClassSplitTest {
  ...
  void testCreateRefactoring() {
    ...
    SimulatorModel model = SimulatorParser.parseSimulator("src/test/resources/layer", InputKind.FEATURE_FILE);
    CtType<?> splitType = model.getSimulatorComponents().iterator().next().getTypes().stream().filter(v -> v.getSimpleName().equals("TestClass")).findFirst().get();
  ...
  void testCreateRefactoring2() {
    ...
    SimulatorModel model = SimulatorParser.parseSimulator("src/test/resources/szenario16/before", InputKind.FEATURE_FILE);
    Set<CtType<?>> types = new HashSet<>(model.getSimulatorComponents().iterator().next().getTypes());
