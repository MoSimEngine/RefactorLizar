...
public class Cycle {
    ...
    private void sort() {
        ...
        while(!availableEdges.isEmpty()) {
            DependencyEdge<CtType<?>> successor = 
            findSuccessor(availableEdges, currentEdge);
    ...
    private  DependencyEdge<CtType<?>> findSuccessor(List<DependencyEdge<CtType<?>>> edges, DependencyEdge<CtType<?>> currentEdge) {
        return edges.stream().filter(v ->  isSuccessor(currentEdge, v)).findAny().orElse(null);

    }
    private boolean isSuccessor(DependencyEdge<CtType<?>> currentEdge, DependencyEdge<CtType<?>> candidate) {
    ...
    String printOnTypeLevel() {
        ...
        return copy.stream().map( v ->
                ...
                + generateTypeMemberString(v.getValue())).collect(Collectors.joining("\n", "\n>>>>>>Cycle Start:\n", "\n<<<<<<<Cycle End" ));
    }
    ...
    String printOnPackageLevel() {
        ...
        for (DependencyEdge<CtType<?>> dependencyEdge : copy) {
            cycles.putIfAbsent(createPackageEntry(dependencyEdge), generateTypeList(dependencyEdge));
            cycles.merge(createPackageEntry(dependencyEdge), generateTypeList(dependencyEdge), mergeLists());
        }
        cycles.entrySet().removeIf(v -> v.getKey().getKey().equals(v.getKey().getValue()));
        return cycles.entrySet().stream().map(v ->
        generateString(v.getKey(), v.getValue()))        
        .collect(Collectors.joining("\n", "\n>>>>>>Cycle Start:\n", "\n<<<<<<<Cycle End" ));
    }
    ...
    private BiFunction<? super List<CtType<?>>, ? super List<CtType<?>>, ? extends List<CtType<?>>> mergeLists() {
    ...
    private List<CtType<?>> generateTypeList(DependencyEdge<CtType<?>> dependencyEdge) {
        return dependencyEdge.getValue().stream().map(member -> member.getMember().getDeclaringType())
            .distinct().collect(Collectors.toList());
    }

    private Entry<CtPackage, CtPackage> createPackageEntry(DependencyEdge<CtType<?>> dependencyEdge) {
        return Map.entry(dependencyEdge.getSource().getPackage(),
        dependencyEdge.getTarget().getPackage());
    }
}
