...
public class TypeLevelReportGeneration {
        ...
        long count = graph.nodes().stream().filter(type -> JavaUtils.isSimulatorType(model, type)).count();
        if(count == 0) {
            ...
            if(JavaUtils.isSimulatorType(model, source)) {
                builder.append(
                    format(
                            ...
                            graph.successors(source).stream().forEach(target ->builder.append(generateMemberUsage(target, graph.edgesConnecting(source, target))));
                }
        }
        return new Report("Language Blob Analyze on type level", builder.toString(), true);
        }

        private static String generateMemberUsage(CtType<?> target, Set<Edge<CtType<?>,CtTypeMember>> causes) {
                return format(
                        "\t%s at positions:\n %s\n\n",
                        target.getQualifiedName(), generatePositionStringMemberLevel(causes));
            }
            private static String generatePositionStringMemberLevel(Set<Edge<CtType<?>,CtTypeMember>> causes) {
