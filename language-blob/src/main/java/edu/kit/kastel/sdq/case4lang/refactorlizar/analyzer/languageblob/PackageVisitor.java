...
public class PackageVisitor extends CtAbstractVisitor {
  ...
  public void visitCtPackage(CtPackage ctPackage) {
    ...
    for (CtType<?> type : ctPackage.getTypes()) {
      ...
      Set<CtPackageReference> packagesOfReferencedTypes = type.getReferencedTypes().stream()
          .filter(v -> v.getPackage() != null).map(v -> v.getPackage()).filter(Objects::nonNull)
          .collect(Collectors.toSet());
      packagesOfReferencedTypes.stream().filter(v -> featureByPackage.containsKey(v.getQualifiedName()))
          .forEach(v -> edges.add(EndpointPair.ordered(new Node(v, featureByPackage.get(v.getQualifiedName())),
  ...
  private List<Node> removeNonBlobs(MutableGraph<Node> graph) {
    return graph.nodes().stream().filter(v -> graph.inDegree(v) > 1).collect(Collectors.toList());
  }
  ...
  private void generateReport(MutableGraph<Node> graph, Collection<Node> result) {
    if (result.isEmpty()) {
      ...
      for (Node node : result) {
        ...
        formattedDescriptions.add(String.format(blobs, node.packag.getQualifiedName(), graph.predecessors(node).stream()
            .map(v -> v.feature.getBundle().getName()).collect(Collectors.joining(","))));
      }
  ...
  private Map<String, Feature> generateFeatureByPackageQName() {
    ...
    for (Feature feature : language.getLanguageFeature()) {
      Collection<CtPackage> packages = feature.getJavaPackage().getElements(new TypeFilter<CtPackage>(CtPackage.class));
