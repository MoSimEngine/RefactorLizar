...
public class PackageLevelReportGeneration {
    ...
    public static Report generateReport(MutableNetwork<CtPackage, Edge<CtPackage, CtType<?>>> graph,
        SimulatorModel model, ModularLanguage language) {
                long count = graph.nodes().stream().filter(type -> JavaUtils.isSimulatorPackage(model, type)).count();
                if(count == 0) {
                ...
                for (CtPackage source : graph.nodes()) {
                        ...
                        builder.append(
                            format(
                                    ...
                                    graph.successors(source).stream().forEach(target ->builder.append(generateTypeUsage(target, graph.edgesConnecting(source, target))));
                }
        }
                return new Report("Language Blob Analyze on package level", builder.toString(), true);
    }

private static String generateTypeUsage(CtPackage target,
                Set<Edge<CtPackage, CtType<?>>> causes) {
        return format("\t%s at classes:\n %s\n\n", target.getQualifiedName(), generatePositionStringMemberLevel(causes));
}
private static String generatePositionStringMemberLevel(Set<Edge<CtPackage,CtType<?>>> causes) {
