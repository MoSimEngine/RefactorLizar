...
public class FeatureViewAdapter {
  ...
  public FeatureView provideFeatureView() {
    ...
    List<Feature> features = modularLanguage //
        .getLanguageFeature() //
        .stream() //
        .map(this::convert) //
        .collect(Collectors.toList());

    List<FeatureToFeatureRelation> featureToFeatureRelations = modularLanguage //
        .getLanguageFeature() //
        .stream() //
  ...
  private List<FeatureToFeatureRelation> findEdges(
    ...
    return input //
        .getBundle() //
        .getInternalRequiredBundles() //
        .stream() //
        .map( //
            internalRequiredBundle -> //
                FeatureToFeatureRelation.of(self, FeatureId.of(internalRequiredBundle)) //
        ).collect(Collectors.toList());
  }
