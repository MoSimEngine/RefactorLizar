...
public class RefatorLizar {
  ...
  private RefatorLizar(ModularLanguage modularLanguage, SimulatorModel simulatorModel) {
  ...
  @Nonnull public static RefatorLizar load( //
    ...
    ModularLanguage modularLanguage = new ModularLanguage( //
        new LanguageParser().parseLanguage( //
            inputSourceModularLanguage.getParameters().get( //
                fileSystemName() //
            ) //
        ) //
    );
    ...
    SimulatorModel simulatorModel = new SimulatorModel( //
        new SimulatorParser().parseLanguage( //
            inputSourceSimulator.getParameters().get( //
                fileSystemName() //
            ) //
        ) //
    );

    return new RefatorLizar(modularLanguage, simulatorModel);
  }
  ...
  private static void checkThatInputSourceTypeIsSupported(InputSource inputSource) {
    ...
    if (!InputSourceType.supports(inputSource.getInputSourceTypeId())) {
      throw new IllegalArgumentException( //
          String.format("Input source type ID %s is not supported.", //
