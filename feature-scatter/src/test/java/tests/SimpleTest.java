...
public class SimpleTest {
  ...
  public void simpleLanguageTest() {
    ...
    feature.add(new Feature(languageModel.getAllPackages().stream().filter(v->v.getQualifiedName().contains("Feature")).findFirst().get(),null));
    ...
    SimulatorModel model1 = new SimulatorModel(simulatorModel.getAllPackages().stream().map(v -> new Feature(v,null)).collect(Collectors.toList()));
